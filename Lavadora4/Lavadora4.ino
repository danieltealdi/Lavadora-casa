...
void loop(){
  ...
  //Serial.println(tic);
    ...
    else if(tiempoPrograma<(tLav+tDes+tAcl)*60000){
      Paso=Aclarado1;
      Lavar();
      }
    else if(tiempoPrograma<(tLav+tDes+tAcl+tDes)*60000){
      Paso=Desague3;
      Desague();
      }
    else if(tiempoPrograma<(tLav+tDes+tAcl+tDes+tAcl)*60000){
      Paso=Aclarado2;
      Lavar();
      }
    else if(tiempoPrograma<(tLav+tDes+tAcl+tDes+tAcl+tDes)*60000){
      Paso=Desague4;
      Desague();
      }
    else if(tiempoPrograma<(tLav+tDes+tAcl+tDes+tAcl+tDes+tAcl)*60000){
      Paso=Aclarado3;
      Lavar();
      }
    else if(tiempoPrograma<(tLav+tDes+tAcl+tDes+tAcl+tDes+tAcl+tDes)*60000){
      Paso=Desague5;
      Desague();
      }
    else if(tiempoPrograma<(tLav+tDes+tAcl+tDes+tAcl+tDes+tAcl+tDes+tCen)*60000){
      Paso=Centrifugado;
      Desague();
      }
    else{
...
if(startflag){
  attachInterrupt(digitalPinToInterrupt(ZERO), Zero_Cross, RISING);
  if(pidError<0){
    myPID.SetTunings(0.70,0.05, 0);
    Tcond=map(Output,0,100,80,600);
    }else{
      myPID.SetTunings(0.95,0,0);
