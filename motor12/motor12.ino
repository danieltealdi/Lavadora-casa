...
void loop(){
  ...
  //unsigned long periodo=micros()-lastflash;
  if(micros()-lastflash>10000){rpm=0;}
  else{
//    pulsosPorVuelta=32;
    rpm=1000000/periodo*60/160;
    }
  tiempoPrograma=millis()-tiempoInicio;
  tic=tic+tiempoPrograma;
  if(tic=1000){
    
    }
  Serial.print("Lastflash: ");
  Serial.print(micros());
  Serial.print(" ");
...
/********PID********/
  if(pidError<0){
    myPID.SetTunings(0.05,0.08, 0);
    Tcond=map(Output,0,100,80,625);
    }else{
      myPID.SetTunings(0.5,0.5, 0);
      Tcond=map(Output,0,100,80,625);
      }
    //myPID.Compute();
  }
